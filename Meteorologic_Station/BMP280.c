...
// registro dos endereços utilizados para fazer as medições de acordo com o datasheet
uint16_t dig_T1,  \
         dig_P1;
...
#elif SUPPORT_32BIT
            *pressure = (BMP280_compensate_P_int32(pRaw));  // in Pa
#endif
        }
    }
    else
...
static int TrimRead(void)
{
	...
	if (HAL_I2C_Mem_Read(&BMP280_I2C, BMP280_ADDRESS, 0x88, 1, trimdata, 25, HAL_MAX_DELAY) != HAL_OK)
	{
			return 1;  //error NUM for debug
	}

	// Read NVM from 0xE1 to 0xE7 (7 bytes)
	if (HAL_I2C_Mem_Read(&BMP280_I2C, BMP280_ADDRESS, 0xE1, 1, trimdata + 25, 7, HAL_MAX_DELAY) != HAL_OK)
	{
		return 2;  //error NUM for debug
	}
